...
```{r}
HB <- read_tsv(here("Data", "qPCR", "p814_air_sampler_comp_vert.txt"))
...
Plate <- read_sheet(G_url, sheet = "qPCR", range = "A3:L11")
...
Meta <- 
  ...
  mutate(Name = gsub("(.+?)_.+", "\\1", Sample),
         ...
         Type = case_when(grepl("POS", Sample) ~ "Pos_control",
                          grepl("BLANK", Sample) ~ "Extr_blank",
...
PCR_fit %>% 
  ...
  ggplot(aes(x = as.factor(Dillution), y= Prediction))+
  geom_line(aes(group = Name), colour = "grey", size = 0.5, 
            position = position_jitter(width = 0.1, seed = 0), alpha = 0.3)+
  geom_point(position = position_jitter(width = 0.1, seed = 0), alpha = 0.4)+
...
PCR_fit %>% 
 ...
 # mutate(CT = c(10)) %>%
  ...
  mutate(CT_diff_exp = rep(c(1, log(5/2,2), 1), 17)) %>% 
  left_join(select(Meta, -Dillution)) %>% 
   ggplot(aes( x = Dillution, y = CT_diff))+
  geom_point(position = position_jitter(width = 0.1), alpha = 0.5)+
  geom_point(data = . %>% filter(Type == "Pos_control"), colour = "orange", size = 3)+
...
#  facet_wrap(~Name)+
  ...
  geom_label(aes(x = 40, y = 80, label = "45 cycles"), colour = "black")+
  ggtitle("Dilution 1:5")

```

choose Samples for Index PCR analysis 
...
PCR_fit %>% 
  ...
  left_join(Meta) %>% 
  filter(Type == "Sample") %>% 
  filter(Dillution == 2 & Type == "Sample") %>% 
  ungroup() %>% 
  arrange(Prediction) %>% 
