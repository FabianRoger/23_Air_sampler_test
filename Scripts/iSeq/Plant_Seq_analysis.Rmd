...
sample.names <- 
   sapply(fnFs, function(x) gsub(".+Plants/(.+)_R1.fastq.gz", "\\1", x), USE.NAMES = FALSE)
...
seqtab.nochim <- read_rds(here("Data", "iSeq", run_name,
                              primer, "seqtab.nochim.rds"))
```
...
I download the PLANiTS database from [here](https://github.com/apallavicini/PLANiTS/blob/master/PLANiTS_29-03-2020.zip) 

The article describing teh database can be found [here](https://academic.oup.com/database/article/doi/10.1093/database/baz155/5722079)
...
system(
  paste(
    ...
    here("Data", "iSeq", run_name, primer, "ASV_swarm.fasta"), "--db", 
     here("Data", "PLANiTS_29-03-2020", "ITS2.SINTAX_format.fas"),
    "--sintax_cutoff 0.1 --tabbedout",
   here("Data","iSeq", run_name, primer, "ITS2_Plants_tax.txt"), sep = " "
)
)
```
...
```{r}
Sintax_taxa <- read_tsv(here("Data","iSeq", run_name,
                             primer, "ITS2_Plants_tax.txt"), 
                        col_names = FALSE) %>% 
  select(1:2)
...
```{r}
write_tsv(Sintax_taxa_table_full, here("Data", "Plants_Sintax_tax.txt"))
write_tsv(Sintax_taxa_table_prob, here("Data", "Plants_Sintax_prob.txt"))

Sintax_taxa_table_full <- read_tsv(here("Data", "Plants_Sintax_tax.txt"))
Sintax_taxa_table_prob <- read_tsv(here("Data", "Plants_Sintax_prob.txt"))
```
...
```{r}
write_tsv(ITS_taxa_80, here("Data","iSeq", run_name, primer, "Plants_Sintax_80.txt"))
ITS_taxa_80 <- read_tsv(here("Data","iSeq", run_name, primer, "Plants_Sintax_80.txt"))
```
...
export
```{r}
write_tsv(ITS_taxa_80_glom, here("Data","iSeq", "Plants_taxa_80_glom.txt"))

write_tsv(ASW_glom, here("Data","iSeq", "Plants_ASW_glom.txt"))
```
