...
11. **Sampling completeness**: fraction of site community captured per sampler × duration. Sites are replicates — do NOT pool over sites. Simple metric:
   - **fraction_detected**: Reference = ALL species detected at a site (pool all samplers × all durations). For each sampler × duration × site, compute what fraction of reference species it detects. This is straightforward species-level overlap: `fraction_detected = S_sampler / S_reference`.
   - Export columns: taxon, Sites, Sampler, duration, S_sampler, S_reference, fraction_detected
